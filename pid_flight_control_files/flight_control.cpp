...
/**Quaternion * _world_to_body_rotation(float * rotational_axis, float theta, float * current_vector) {
    ...
    **
    ...
    q_vector->a = q->a*vector->a - q->b*vector->b - q->c*vector->c - q->d*vector->d;
    q_vector->b = q->a*vector->b + q->b*vector->a + q->c*vector->d - q->d*vector->c;
    q_vector->c = q->a*vector->c - q->b*vector->d + q->c*vector->a + q->d*vector->b;
    q_vector->d = q->a*vector->d + q->b*vector->c - q->c*vector->b + q->d*vector->a;
    

    q_vector_q_star->a = q_vector->a*vector->a - q_vector->b*vector->b - q_vector->c*vector->c - q_vector->d*vector->d;
    q_vector_q_star->b = q_vector->a*vector->b + q_vector->b*vector->a + q_vector->c*vector->d - q_vector->d*vector->c;
    q_vector_q_star->c = q_vector->a*vector->c - q_vector->b*vector->d + q_vector->c*vector->a + q_vector->d*vector->b;
    q_vector_q_star->d = q_vector->a*vector->d + q_vector->b*vector->c - q_vector->c*vector->b + q_vector->d*vector->a;
...
Point * world_to_body(float theta, Point * reference_point, float px, float py) {
    ...
    Point * body;
    float rotation_matrix[[]] = [[cos(theta), sin(theta), px], [-sin(theta), cos(theta), py], [0, 0, 1]];
    body = rotation_matrix*reference_point;
    return body;
