...
void pwm1__init_single_edge(uint32_t frequency_in_hertz) {
  ...
  const uint32_t match_reg_value =
      (clock__get_peripheral_clock_hz() / valid_frequency_in_hertz);
  ...
  // MR0 = 10/2 = 5. This means, TC will count upto 5 per pulse, generating 2
  // pulse per second
  ...
  LPC_PWM1->PCR |=
      (pwm_channel_output_enable_mask << 9); ///< Enable the PWM (bits 9-14)
}
