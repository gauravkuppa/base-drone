...
the matrix is an array of array pointers where each array pointer corresponds to
a vector
*/
...
static void scalar_vector_multiplication(double factor, double *vector,
                                         int length);
...
/* matrix m will become the identity so the caller must save their matrix
 * themselves  */
...
Matrix *multiply(Matrix *m1, Matrix *m2) {
  ...
  for (i = 0; i < product->columns; i++) {
    for (j = 0; j < product->rows; j++) {
      product->numbers[i][j] =
          vector_multiply(trans->numbers[j], m2->numbers[i], product->rows);
    }
  }
...
Matrix *gram_schmidt(Matrix *m) {
  ...
  if (m != NULL || m->rows == m->columns || zero_vector(m) != 1) {
    ...
    for (i = 1; i < m->columns; i++) {
      ...
      ortho->columns++;
      ortho->numbers =
          realloc(ortho->numbers, sizeof(double *) * ortho->columns);
...
double *projection(Matrix *m, double *v, int length) {
  ...
  for (i = 0; i < m->columns; i++) {
    ...
    vector = copy;
    factor = vector_multiply(v, vector, m->rows) /
             vector_multiply(vector, vector, m->rows);
...
Matrix *solved_aug_matrix(Matrix *m) {
  ...
  for (k = 0; k < absolute; k++) {
    ...
    /* now finish the upper triangle  */
    for (i = (low->rows > low->columns) ? low->columns - 1 : low->rows - 1;
         i > 0; i--) {
      for (j = i - 1; j >= 0; j--) {
        if (low->numbers[i][i] == 0)
          continue;
